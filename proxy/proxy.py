...
class Proxy:
	...
	def listen_for_messages(self):
		try:
			...
			self.logger.log_debug_message('[*] Shutting down proxy')
			self.exit_proxy()
		except socket.error:
			return
	...
	def check_ready_connections(self, readable):
		for sock in readable:
			if sock is self.listening_socket:
				...
				self.logger.log_debug_message("[*] Received Connection")
	...
	def handle_connection(self, conn):
		try:
			...
			self.logger.log_error_message("*** Error, received invalid data from client ***")
			#conn.close()	#OR RETURN?
			return
		except DisconnectException:
			self.logger.log_debug_message("***Client has closed connection***")
	...
	def send_request_and_response(self):
		while self.running:
			...
			try:
				...
				self.logger.log_error_message("*** Error, lost connection to server ***")
				self.exit_proxy()
				return
			except struct.error:
				self.logger.log_error_message("*** Error, received invalid data from server ***")
				continue
	...
	def send_response_to_client(self, conn, response):
		try:
			...
			self.logger.log_debug_message('***Cant send response. Closed connection***')
			self.remove_client(conn)
	...
	def set_up_sockets(self):
		try:
			...
			self.logger.log_error_message('*** Error creating a server socket: %s ***' % e)
			self.failure_exit_proxy(e)
		self.logger.log_debug_message('[*] Server socket initialized...')

		try:
			server = (self.server_address, self.server_port)
			self.server_socket.connect(server)
		except socket.error as e:
			self.logger.log_error_message('*** Error connecting to server: %s ***' % e)
			self.failure_exit_proxy(e)
		self.logger.log_debug_message('[*] Connection to server estabilished...')
		...
		try:
			...
			self.logger.log_error_message('*** Error creating a listening socket: %s ***' % e)
			self.failure_exit_proxy(e)
		self.logger.log_debug_message('[*] Listening socket initialized...')

		try:
			self.listening_socket.bind((self.host_address, self.host_port))
			self.listening_socket.listen(5000)
		except socket.error as e:
			self.logger.log_error_message('*** Error binding a listening socket: %s ***' % e)
			self.failure_exit_proxy(e)
		self.logger.log_debug_message('[*] Listening socket binded successfully...')

		self.logger.log_debug_message('[*] Proxy initialized successfully!')
